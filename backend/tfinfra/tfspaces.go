...
type Spaces struct {
	...
	CorsRule      *[]CorsRule    `json:"cors_rule" description:"(Optional) A rule of Cross-Origin Resource Sharing (documented below)." validate:"omitempty,dive"`
	LifecycleRule *LifecycleRule `json:"lifecycle_rule" description:"(Optional) A configuration of object lifecycle management (documented below)." validate:"omitempty,dive"`
...
type LifecycleRule struct {
	...
	NoncurrentVersionExpiration        *NoncurrentVersionExpiration `json:"noncurrent_version_expiration" description:"(Optional) Specifies when non-current object versions expire (documented below)." validate:"omitempty,dive,required"`
}
